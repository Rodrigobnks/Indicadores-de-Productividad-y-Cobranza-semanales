...
def aplicar_formato_tabla_resumen_mixto(df_tabla: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, fila in df_fmt.iterrows():
        indicador = str(fila.get("Indicador", ""))
        es_pct = indicador.strip().startswith("%") or "cumplimiento" in normalizar_texto_tc(indicador).lower()
